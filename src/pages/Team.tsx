              ...
              >
                      ...
                      <div className="mt-4 flex gap-5">
                        {member.linkedin && (
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 text-xs text-[#B8C5D0] hover:text-[#00D4FF] transition-colors"
                          >
                ...
                <div className="mt-6 space-y-4">
                  {member.bio.map((paragraph, i) => (
                    <p key={i} className="text-[#B8C5D0] text-sm leading-relaxed">
                      {paragraph}
                    </p>
                  ))}
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-[#2A3040]">
                  <p className="text-xs uppercase tracking-widest text-[#B8C5D0] mb-3">
                    Certifications
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {member.certifications.map((cert) => (
                      <span
                        key={cert}
                        className="rounded-full border border-[#2A3040] bg-[#232936] px-3 py-1 text-xs text-[#B8C5D0]"
                      >
                        {cert}
                      </span>
                    ))}
...
function Avatar({ photo, name }: { photo?: string; name: string }) {
  ...
  return (
    <div className="h-28 w-28 rounded-full bg-[#232936] border-2 border-[#00D4FF]/30 flex items-center justify-center text-4xl text-[#B8C5D0]">
      👤
    </div>
  );
}
